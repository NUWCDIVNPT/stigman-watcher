...
describe("setup and teardown", function () {
  ...
  describe("Event Mode, stop api and go api offline, come back up", async function () {
    this.timeout(120_000)
    let db, auth, api
    let watcher
    const env = {
  ...
  this.timeout(120_000)
    ...
    it("should retry 7 times at 3 seconds between retries and exit", async () => {
      ...
      // ensure retries are spaced by less than 7 seconds
      for (let i = 1; i < authRetryLogs.length; i++) {
        const prev = new Date(authRetryLogs[i - 1].timestamp).getTime()
        const cur = new Date(authRetryLogs[i].timestamp).getTime()
        const diff = cur - prev
        expect(diff).to.be.lessThan(env.retryDelay + 2000) // allow 2 seconds of wiggle room
      }
