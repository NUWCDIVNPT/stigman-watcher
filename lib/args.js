...
.option('--retry-interval <ms>', 'Interval in milliseconds between connectivity retry attempts when an alarm condition is raised (`WATCHER_RETRY_INTERVAL`).', parseIntegerArg, parseIntegerEnv(pe.WATCHER_RETRY_INTERVAL) ?? 5000)
.option('--retry-count <number>', 'Maximum number of connectivity retry attempts when an alarm condition is raised (`WATCHER_RETRY_COUNT`). Use 0 to retry forever.', parseIntegerArg, parseIntegerEnv(pe.WATCHER_RETRY_COUNT) ?? 5)
