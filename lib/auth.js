...
async function authenticateClientSecret () {
  ...
  const response = await got.post(self.url, requestOptions)
  logResponse(response)
  return response.body
}
...
function logResponse (response) {
  logger.http({
    ...
    response: {
      ...
      body: {...response.body, access_token: true, id_token: true}
    }
  })
}

/**
 * Handler for when 'authOffline' alarm is raised.
 * Uses options.retryInterval (ms) and options.retryCount (0 = forever)
 */
let alarmRetryAttempts = 0
function offlineRetryHandler() {
  alarmRetryAttempts++
  logger.info({
    component,
    message: 'Testing if OIDC Provider is online',
    attempt: alarmRetryAttempts
  })
  getOpenIDConfiguration()
  .then(() => {
    alarmRetryAttempts = 0
    Alarm.authOffline(false)
  })
  .catch(() => {
    // if retryCount > 0 and we've reached or exceeded it, shutdown
    if (options.retryCount > 0 && alarmRetryAttempts >= options.retryCount) {
      logger.info({
        component,
        message: 'OIDC Provider connectivity maximum tries reached, requesting shutdown',
        attempts: alarmRetryAttempts
      })
      Alarm.shutdown(CONSTANTS.ERR_AUTHOFFLINE)
    }
    else {
      setTimeout(offlineRetryHandler, options.retryInterval)
    }
  })
}
