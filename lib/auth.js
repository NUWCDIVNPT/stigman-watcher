...
module.exports.getToken = async function getToken( log = false ) {
  try {
    ...
    if (log) {
      logger.log( { level: 'http', ...{ component: 'auth', message: `OIDC token request`, request: { clientId: config.clientId, authentication: self.authentication, method: 'POST', url: self.url} } })
    }
    self.tokens = await authenticateClient()
    self.tokenDecoded = self.decodeToken(self.tokens.access_token)
    if (log) {
      logger.http({ component: 'auth', message: `OIDC token response`, payload: self.tokenDecoded })
    }
    return self.tokenDecoded
  }
  catch (e) {
