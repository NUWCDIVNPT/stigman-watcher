...
async function writer ( target ) {
  ...
  try {
    ...
    if ( !target.knownAsset ) {
      ...
      console.log( `[${component}] ${target.assetProps.name} ${r.created ? 'created as' : 'found as '} ${JSON.stringify(r.apiAsset)}` )
    ...
    if ( target.knownAsset && target.hasNewAssignment ) {
      ...
      console.log(`[${component}] ${r.name} (id ${r.assetId}): STIG assignments updated to ${JSON.stringify(stigs)}`)
    }
    ...
    if (reviews.length > 0) {
      ...
      console.log(`[${component}] ${target.assetProps.name} (id ${target.assetProps.assetId}): posted reviews, permitted: ${r.permitted.length}, rejected: ${r.rejected.length}, errors: ${r.errors.length}`)  
    }
    console.log(`[${component}] ${target.assetProps.name} finished`)
  }  
  catch (e) {
    console.log(`[${component}] ${target.assetProps.name} error ${e}`)
  }
}
