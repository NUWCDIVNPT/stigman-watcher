...
import { createOrGetAsset, patchAsset, postReviews, getCollectionAssets, getInstalledStigs } from './api.js'
...
async function writer ( taskAsset ) {
  ...
  try {
    ...
    if ( !taskAsset.knownAsset ) {
      const r = await createOrGetAsset( taskAsset.assetProps )
    ...
    if ( taskAsset.knownAsset && taskAsset.hasNewAssignment ) {
      const r = await patchAsset(taskAsset.assetProps.assetId, {
    ...
    if (reviews.length > 0) {
      const r = await postReviews(options.collectionId, taskAsset.assetProps.assetId, reviews)
      logger.info({
        component: component, 
...
async function resultsHandler( parsedResults, cb ) {
  ...
  try {
    ...
    const apiAssets = await getCollectionAssets(options.collectionId)
    const apiStigs = await getInstalledStigs()
