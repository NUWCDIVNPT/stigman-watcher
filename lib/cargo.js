...
class TaskObject {
  constructor ({ apiAssets = [], apiStigs = [], parsedResults = [] }) {
    ...
    for (const apiAsset of apiAssets) {
      ...
      if (apiAsset.metadata?.cklHostName) {
        const v = this.mappedCklHostnames.get(
          apiAsset.metadata.cklHostName.toLowerCase()
        )
        if (v) {
          v.push(apiAsset)
        } else {
          this.mappedCklHostnames.set(
            apiAsset.metadata.cklHostName.toLowerCase(),
            [apiAsset]
          )
        }
  ...
  _findAssetFromParsedTarget (target) {
    ...
    const matchedByCklHostname = this.mappedCklHostnames.get(
      target.metadata.cklHostName.toLowerCase()
    )
    if (!matchedByCklHostname) return null
    const matchedByAllCklMetadata = matchedByCklHostname.find(
      asset =>
        asset.metadata.cklWebDbInstance?.toLowerCase() ===
          target.metadata.cklWebDbInstance?.toLowerCase() &&
        asset.metadata.cklWebDbSite?.toLowerCase() ===
          target.metadata.cklWebDbSite?.toLowerCase()
    )
  ...
  _createTaskAssets () {
    ...
    // if parsedResults is an object convert it to array (cargosize=1)
    this.parsedResults = Array.isArray(this.parsedResults)
      ? this.parsedResults
      : [this.parsedResults]
    ...
    for (const parsedResult of this.parsedResults) {
      ...
      let mapKey,
        tMeta = parsedResult.target.metadata
      if (!tMeta.cklHostName) {
        mapKey = parsedResult.target.name.toLowerCase()
      } else {
        mapKey = `${tMeta.cklHostName}-${tMeta.cklWebDbSite ?? 'NA'}-${
          tMeta.cklWebDbInstance ?? 'NA'
        }`
      }
      ...
      if (!apiAsset && !config.createObjects) {
        ...
        logger.warn({
          ...
          reason:
            'asset does not exist for target and options.createObjects == false',
          file: parsedResult.file,
          target: parsedResult.target
        })
        continue
      ...
      const stigIsInstalled = ({ benchmarkId, revisionStr }) => {
        ...
        if (revisionStrs) {
          return revisionStr && config.strictRevisionCheck
            ? revisionStrs.includes(revisionStr)
            : true
        } else {
          return false
        }
      ...
      const stigIsNewlyAssigned = benchmarkId =>
        taskAsset.newAssignments.includes(benchmarkId)
...
async function writer (taskAsset) {
  ...
  try {
    ...
    if (!taskAsset.knownAsset) {
      ...
      logger.info({
        component: component,
        message: `asset ${r.created ? 'created' : 'found'}`,
        asset: r.apiAsset
      })
    ...
    if (taskAsset.knownAsset && taskAsset.hasNewAssignment) {
      ...
      logger.info({
        component: component,
        message: `STIG assignments updated`,
    ...
    if (reviews.length > 0) {
      const r = await api.postReviews(
        config.collectionId,
        taskAsset.assetProps.assetId,
        reviews
      )
      logger.info({
        component: component,
        ...
        asset: {
          name: taskAsset.assetProps.name,
          id: taskAsset.assetProps.assetId
        },
        rejected: r.rejected,
        affected: r.affected
      })
