...
async function writer ( target ) {
  ...
  try {
    log( 'info', { component: component, message: `${target.assetProps.name} started` })
    ...
    if ( !target.knownAsset ) {
      ...
      log ( 'success', { component: component, message: `Asset ${r.created ? 'created' : 'found'}`, asset: r.apiAsset })
    ...
    if ( target.knownAsset && target.hasNewAssignment ) {
      ...
      log( 'success', { component: component, message: `STIG assignments updated`, asset: r })
    }
