;(function (exports) {
  exports.reviewsFromCkl = function ({
    ...
    const normalizeKeys = function (input) {
      ...
      return Object.keys(input).reduce(function (newObj, key) {
        let val = input[key]
        let newVal =
          typeof val === 'object' && val !== null ? normalizeKeys(val) : val
    ...
    const resultEngineCommon = comments?.length
      ? processRootXmlComments(comments)
      : null
    ...
    function processIStig (iStigElement) {
      ...
      iStigElement.forEach(iStig => {
        let checklist = {}
        // get benchmarkId
        let stigIdElement = iStig.STIG_INFO[0].SI_DATA.filter(
          d => d.SID_NAME === 'stigid'
        )?.[0]
        checklist.benchmarkId = stigIdElement.SID_DATA.replace(
        ...
        const stigVersionData = iStig.STIG_INFO[0].SI_DATA.filter(
          d => d.SID_NAME === 'version'
        )?.[0].SID_DATA
        let stigVersion = stigVersionData.match(/(\d+)/)?.[1]
        let stigReleaseInfo = iStig.STIG_INFO[0].SI_DATA.filter(
          d => d.SID_NAME === 'releaseinfo'
        )?.[0].SID_DATA
        const stigRelease = stigReleaseInfo.match(/Release:\s*(.+?)\s/)?.[1]
        const stigRevisionStr =
          stigVersion && stigRelease ? `V${stigVersion}R${stigRelease}` : null
    ...
    function generateReview (vuln, resultEngineCommon) {
      ...
      let detail =
        vuln.FINDING_DETAILS.length > maxCommentLength
          ? vuln.FINDING_DETAILS.slice(0, maxCommentLength)
          : vuln.FINDING_DETAILS
      ...
      let comment =
        vuln.COMMENTS.length > maxCommentLength
          ? vuln.COMMENTS.slice(0, maxCommentLength)
          : vuln.COMMENTS
      if (!vuln.COMMENTS) {
          ...
          for (const comment of vuln['__comment']) {
            if (comment.toString().startsWith('<Evaluate-STIG>')) {
              ...
              try {
                ...
                console.log(
                  `Failed to parse Evaluate-STIG VULN XML comment for ${ruleId}`
                )
              }
    ...
    function bestStatusForReview (review) {
      ...
      switch (fieldSettings.detail.required) {
        ...
        case 'findings':
          if (
            review.result !== 'fail' ||
            (review.result === 'fail' && review.detail)
          ) {
            detailSubmittable = true
          }
          break
      ...
      switch (fieldSettings.comment.required) {
        ...
        case 'findings':
          if (
            review.result !== 'fail' ||
            (review.result === 'fail' && review.comment)
          ) {
            commentSubmittable = true
          }
          break
      ...
      const resultSubmittable =
        review.result === 'pass' ||
        review.result === 'fail' ||
        review.result === 'notapplicable'
  ...
  exports.reviewsFromXccdf = function ({
    ...
    if (!parsed.Benchmark && !parsed.TestResult)
      throw new Error('No Benchmark or TestResult element')
    if (parsed.Benchmark) {
      if (!parsed.Benchmark.TestResult)
        throw new Error('No Benchmark.TestResult element')
      if (!parsed.Benchmark.TestResult['target'])
        throw new Error('No Benchmark.TestResult.target element')
      if (!parsed.Benchmark.TestResult['rule-result'])
        throw new Error('No Benchmark.TestResult.rule-result element')
      testResult = parsed.Benchmark.TestResult
      benchmarkId = parsed.Benchmark.id.replace(
        'xccdf_mil.disa.stig_benchmark_',
        ''
      )
    } else {
      if (!parsed.TestResult['benchmark'])
        throw new Error('No TestResult.benchmark element')
      if (!parsed.TestResult['target'])
        throw new Error('No TestResult.target element')
      if (!parsed.TestResult['rule-result'])
        throw new Error('No TestResult.rule-result element')
      testResult = parsed.TestResult
      let benchmarkAttr
      if (
        testResult.benchmark.id?.startsWith('xccdf_mil.disa.stig_benchmark_')
      ) {
        benchmarkAttr = testResult.benchmark.id
      } else if (
        testResult.benchmark.href?.startsWith('xccdf_mil.disa.stig_benchmark_')
      ) {
    ...
    function generateReview (ruleResult, resultEngineCommon) {
      ...
      if (resultEngineCommon) {
        if (resultEngineCommon.product === 'stig-manager') {
          ...
          const checkContentHref = ruleResult?.check?.[
            'check-content-ref'
          ]?.href?.replace('#scap_mil.disa.stig_comp_', '')
          const checkContentName = ruleResult?.check?.[
            'check-content-ref'
          ]?.name?.replace('oval:mil.disa.stig.', '')
      ...
      for (const field of fields) {
        switch (fieldSettings[field].required) {
          ...
          case 'findings':
            commentsSubmittable =
              review.result !== 'fail' ||
              (review.result === 'fail' && review[field])
            break
          case 'always':
    ...
    function processTargetFacts (targetFacts) {
      ...
      const reTagAsset =
        /^tag:stig-manager@users.noreply.github.com,2020:asset:(.*)/
    ...
    function processTarget (testResult) {
      const assetFromFacts = processTargetFacts(
        testResult['target-facts']?.fact
      )
      return {
  ...
  exports.reviewsFromCklb = function ({
    ...
    function processStigs (stigs) {
      ...
      for (const stig of stigs) {
        ...
        // }
        const checklist = {}
        checklist.benchmarkId =
          typeof stig?.stig_id === 'string'
            ? stig.stig_id.replace('xccdf_mil.disa.stig_benchmark_', '')
            : ''
        const stigVersion = '0'
        const stigRelease =
          typeof stig?.release_info === 'string'
            ? stig.release_info.match(/Release:\s*(.+?)\s/)?.[1]
            : ''
        checklist.revisionStr =
          checklist.benchmarkId && stigRelease
            ? `V${stigVersion}R${stigRelease}`
            : null
    ...
    function generateReview (rule, resultEngineCommon) {
      ...
      let detail =
        rule.finding_details?.length > maxCommentLength
          ? rule.finding_details.slice(0, maxCommentLength)
      ...
      let comment =
        rule.comments?.length > maxCommentLength
          ? rule.comments.slice(0, maxCommentLength)
          : rule.comments
      if (!rule.comments) {
        switch (importOptions.emptyComment) {
          case 'ignore':
