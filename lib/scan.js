...
function tryScheduleNextScan() {
  if (!isParseQueueActive && !isCargoQueueActive) {
    if (options.oneShot) {
      // In one-shot mode with no work, cargoQueue.on('drain') is never called, so we must exit (no history to write)
      logger.info({component, message: 'finished one shot mode - no work to do'})
